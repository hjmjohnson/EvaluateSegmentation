...
TestLesion* findNearestTestLesion(GTLesion *truth, vector<TestLesion> *lesions){
	...
	TestLesion * nearest=ITK_NULLPTR;
...
{
	LesionDetectionMask * organmask=ITK_NULLPTR;
	string l_truth_file = "";
	string l_test_file = "";
	...
	if(maskFile!=ITK_NULLPTR && std::strlen(maskFile) > 0){
